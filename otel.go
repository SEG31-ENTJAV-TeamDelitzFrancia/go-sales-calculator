...
func newTracerProvider(ctx context.Context) (*trace.TracerProvider, error) {
	// initialise exporter
	otelTraceExporter, err := otlptracegrpc.New(ctx, otlptracegrpc.WithInsecure())
	if err != nil {
		return nil, err
	}
	...
	tracerProvider := trace.NewTracerProvider(
		trace.WithBatcher(otelTraceExporter,
...
func newMeterProvider(ctx context.Context) (*metric.MeterProvider, error) {
	// initialise exporter
	otelMetricExporter, err := otlpmetricgrpc.New(ctx, otlpmetricgrpc.WithInsecure())
	if err != nil {
		return nil, err
	}
...
func newLoggerProvider(ctx context.Context) (*log.LoggerProvider, error) {
	...
	// initialise otel exporter
	otelLogExporter, err := otlploggrpc.New(ctx, otlploggrpc.WithInsecure())
	if err != nil {
		return nil, err
	}
	...
	loggerProvider := log.NewLoggerProvider(
		...
		log.WithProcessor(log.NewBatchProcessor(otelLogExporter)),
	)
